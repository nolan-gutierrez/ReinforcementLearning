...
class Plot:
    ...
    def plotList(self,myList):
        ...
        plt.legend()
        plt.show()
    def plotListGeneral(self,myLists,labels, xlabel,ylabel,title):
        lineHs = {}
        print("myLists is: ", myLists)
        print("myLists[0]: ", myLists[0])
        for i,item in enumerate(myLists):
            print( "i: ", i)
            lineHs[i] = plt.plot(item, label = labels[i])
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.title(title)
        plt.legend()
